...
- Proto：官方 schema 來源 `mexcdevelop/websocket-proto`；建議在 Python 端用 `protobuf==4.25.1`（requirements.txt 已鎖定）解析再轉 JSON。
...
- `src/app/infrastructure/external/mexc/ws/ws_client.py` 以 `MEXCWebSocketClient` 包裝；keepalive 預設 25 分鐘（25*60s）、heartbeat 20s，並禁用 websockets 內建 ping。
