...
$MEXC_API_SECRET = (Select-String -Path .env -Pattern '^MEXC_API_SECRET=').ToString().Split('=')[1].Trim()
...
echo $MEXC_API_SECRET | gcloud secrets versions add mexc-api-secret --data-file=-
...
gcloud run deploy qrl-trading-api `
  ...
  --set-secrets "MEXC_API_KEY=mexc-api-key:latest,MEXC_API_SECRET=mexc-api-secret:latest,REDIS_URL=redis-url:latest"
